...
if (requestNode) {
  requestNode.parameters.jsCode = requestNode.parameters.jsCode
    ...
    .replace("  '只返回完整章节正文，不要解释写作过程，不要另附审计数据。',", "  '研究过程先于写作：先把当前研究任务改写成一个真正需要作出取舍的问题，提出可能成立的解释或路径，再寻找能够区分它们的证据。不要从已有指标直接跳到建议。',\n  '分析关系和机制：说明对象之间如何发生作用、哪些条件是必要前提、相关性为何可能不是因果。主动寻找至少一种能解释同一现象的替代解释；只有存在真实取舍时才比较方案，不为了形式强行凑选项。',\n  '本章必须推进一个新的决策边界：把 previous_decisions 当作上游待检验前提，先说明哪些问题已经由上游处理、哪些仍未解决，再把证据用于本章的新选择。不要把上游的完整论证重新改写成背景综述，也不要提前替后续章节完成它们的核心判断；只有当新证据改变上游判断时，才重复并解释改变。',\n  '成稿前做一次自我挑战：当前判断最脆弱的环节是什么，什么事实会推翻它，它对下一章节究竟形成了什么约束。把这段阶段性结论写入 decision_brief，供后续章节直接承接；decision_brief 是自由文本，不使用固定模板。',\n  'reader_chapter 是面向甲方的完整正文，可以自由组织，但应让读者看清本章新增的问题、关系、取舍和决定，而不只是数据摘要后接建议。只返回 decision_brief 与 reader_chapter。',")
