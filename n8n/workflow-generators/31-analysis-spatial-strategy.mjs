const steps = [
  {
    order: 1,
    key: 'step_01_policy_site',
    title: '政策与场地',
  },
  {
    order: 2,
    key: 'step_02_regional_role',
    title: '区域角色',
  },
  {
    order: 3,
    key: 'step_03_market_flow',
    title: '市场与流动',
  },
  {
    order: 4,
    key: 'step_04_supply_gap',
    title: '供给与空位',
  },
  {
    order: 5,
    key: 'step_05_audience_use',
    title: '客群与使用',
  },
  {
    order: 6,
    key: 'step_06_theme_resources',
    title: '主题与资源',
  },
  {
    order: 7,
    key: 'step_07_positioning',
    title: '项目定位',
  },
  {
    order: 8,
    key: 'step_08_product_mix',
    title: '产品组合',
  },
  {
    order: 9,
    key: 'step_09_spatial_layout',
    title: '空间布局',
  },
  {
    order: 10,
    key: 'step_10_operating_model',
    title: '运营模式',
  },
  {
    order: 11,
    key: 'step_11_financial_check',
    title: '财务校验',
  },
  {
    order: 12,
...
SET workflow_execution_id = $3::text, status = 'running', current_step = 'step_01_policy_site',
  request = $2::jsonb, started_at = COALESCE(started_at, NOW()), updated_at = NOW()
...
nodes.push({
  ...
  SET status = 'failed',
    ...
    updated_at = NOW()
  FROM failed_run
  WHERE step.run_id = failed_run.id AND step.step = failed_run.current_step
  RETURNING step.step
)
SELECT id::text AS run_id, status, current_step, error, finished_at FROM failed_run;`,
      options: { queryReplacement: '={{ [$(\'Normalize Analysis Request\').first().json.run_id, JSON.stringify($json.error ?? $json)] }}' },
    },
    [12180, 540],
...
let previous = 'Claim Queued Analysis Run';
for (const step of steps) {
  const x = 1380 + (step.order - 1) * 880;
  const configureName = `Configure ${step.key}`;
  const queryName = `Query Project Data ${step.key}`;
  const attachName = `Attach Project Data ${step.key}`;
  const executeName = `Execute ${step.key}`;
  nodes.push({
    ...node(
      configureName,
      'n8n-nodes-base.code',
...
if (previous.status && previous.status !== 'completed') throw new Error('previous_step_failed:' + String(previous.step_key ?? 'previous_step'));
...
return [{ json: {
  ...
  step_key: ${JSON.stringify(step.key)},
  step_title: ${JSON.stringify(step.title)},
} }];`,
      },
      [x, 300],
      `configure-step-${String(step.order).padStart(2, '0')}-000000000000`,
    ),
    onError: 'continueErrorOutput',
    retryOnFail: true,
    maxTries: 2,
  ...
  nodes.push({
    ...node(
      ...
      {
        method: 'POST',
        url: '=__SPATIAL_API_BASE_URL__/analysis/spatial-strategy/project-data/step',
        authentication: 'genericCredentialType',
        genericAuthType: 'httpHeaderAuth',
        sendBody: true,
        contentType: 'json',
        specifyBody: 'json',
        jsonBody: '={{ JSON.stringify({ history_id: $json.history_id, step_key: $json.step_key, project_context: $json.project_context }) }}',
        options: { timeout: 600000, response: { response: { responseFormat: 'json' } } },
      },
      [x + 220, 300],
      `query-project-data-${String(step.order).padStart(2, '0')}-00000000000000`,
    ),
    credentials: { httpHeaderAuth: { id: 'n8n-webhook-client', name: 'n8n Webhook Client' } },
    onError: 'continueErrorOutput',
    retryOnFail: true,
  ...
  nodes.push(node(
    ...
    {
      mode: 'runOnceForAllItems',
      jsCode: `const state = $('${configureName}').first().json;
const response = $input.first()?.json ?? {};
const projectData = response.status === 'success' ? response : {
  status: 'unavailable',
  history_id: state.history_id,
  step_key: state.step_key,
  citations: [],
  queries: [],
  warnings: [String(response.error?.message ?? response.message ?? response.error ?? 'project_data_unavailable')],
};
return [{ json: { ...state, project_data: projectData } }];`,
    },
    [x + 440, 300],
    `attach-project-data-${String(step.order).padStart(2, '0')}-0000000000000`,
  ));
  nodes.push({
    ...node(
      executeName,
      ...
      {
        source: 'database',
        workflowId: { __rl: true, value: 'analysisDecisionStep01', mode: 'id' },
      ...
      [x + 660, 300],
      `execute-step-${String(step.order).padStart(2, '0')}-0000000000000`,
    ),
    onError: 'continueErrorOutput',
    retryOnFail: true,
    maxTries: 2,
  ...
  connections[previous] = previous === 'Claim Queued Analysis Run'
    ? { main: [[{ node: configureName, type: 'main', index: 0 }]] }
    : { main: [[{ node: configureName, type: 'main', index: 0 }], [{ node: 'Fail Analysis Run', type: 'main', index: 0 }]] };
  connections[configureName] = {
    main: [
      [{ node: queryName, type: 'main', index: 0 }],
      [{ node: 'Fail Analysis Run', type: 'main', index: 0 }],
    ],
  };
  connections[queryName] = {
    main: [
      [{ node: attachName, type: 'main', index: 0 }],
      [{ node: attachName, type: 'main', index: 0 }],
    ],
  };
  connections[attachName] = { main: [[{ node: executeName, type: 'main', index: 0 }]] };
  previous = executeName;
}
...
const visualSchema = { type: 'object', properties: { title: { type: 'string' }, format: { type: 'string', enum: ['map', 'chart', 'table'] }, rationale: { type: 'string' }, dataset_id: { type: 'string' }, layers: { type: 'array', items: layerSchema }, group_by: { type: 'string' }, metric_op: { type: 'string', enum: ['count', 'sum', 'avg'] }, metric_field: { type: 'string' } }, required: ['title', 'format', 'rationale', 'dataset_id', 'layers', 'group_by', 'metric_op', 'metric_field'], additionalProperties: false };
const schema = { type: 'object', properties: { visuals: { type: 'array', minItems: 1, maxItems: 6, items: visualSchema } }, required: ['visuals'], additionalProperties: false };
const instructions = ['你是城市空间分析的图件设计 Agent。阅读十二项分析结论后，为本项目设计 2 到 6 张真正有解释力的数据图或表。', '不是从固定模板中选图：你要为每项输出自行确定标题、形式、数据集、地图叠加图层、聚合字段和指标。', '渲染器会读取每个指定数据集的完整项目数据包，再进行绘制或聚合。不得要求抽样、截断、AI 绘画或编造空间事实。'].join('\\n');
return [{ json: {
  ...completed,
  ...
  instructions,
  input: JSON.stringify({ project_question: request.project_question, completed_analyses: steps, available_datasets: datasets }),
...
const response = $input.first()?.json ?? {};
let parsed;
try { parsed = JSON.parse(String(response.output_text ?? '')); } catch { throw new Error('visual agent returned invalid JSON'); }
const visuals = Array.isArray(parsed.visuals) ? parsed.visuals.slice(0, 6) : [];
if (!visuals.length) throw new Error('visual agent designed no visuals');
...
const response = $input.first()?.json ?? {};
let parsed;
try { parsed = JSON.parse(String(response.output_text ?? '')); } catch { throw new Error('report editorial returned invalid JSON'); }
const editorialNarrative = String(parsed.narrative ?? '').trim();
if (!editorialNarrative) throw new Error('report editorial returned empty narrative');
...
connections[previous] = {
  main: [
    [{ node: 'Build Visual Agent Request', type: 'main', index: 0 }],
    [{ node: 'Fail Analysis Run', type: 'main', index: 0 }],
  ],
};
...
connections['发送报告到飞书？'] = {
  main: [
    [{ node: '发送报告到飞书', type: 'main', index: 0 }],
    [{ node: 'Complete Analysis Run', type: 'main', index: 0 }],
  ],
};
connections['发送报告到飞书'] = {
  main: [
...
export default {
  id: 'analysisSpatialStrategy1',
  name: 'AN-20 Twelve-Direction Spatial Analysis',
  active: true,
  nodes,
  connections,
  settings: { executionOrder: 'v1', executionTimeout: 14400 },
  versionId: 'ba20956a-698c-44df-bd7b-9376372280e0',
