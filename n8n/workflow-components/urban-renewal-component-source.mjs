...
const completedAnalyses = Object.entries(steps).map(([stepKey, value]) => ({ step_key: stepKey, title: String(value?.title ?? ''), reader_chapter: String(value?.reader_chapter ?? '').trim(), evidence_status: String(value?.quality_gate?.evidence_status ?? ''), evidence_count: Number(value?.quality_gate?.evidence_count ?? 0) })).filter((item) => item.reader_chapter);
