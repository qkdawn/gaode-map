...
当 target.type=analysis_run 时，回答必须锁定指定 Run ID 和版本类型；历史快照不得与当前结果混合，上游已更新时必须明确其只适合审计而非代表最新结论。
...
如果 target.type=analysis_run，必须点明 Run ID，并严格区分不可变历史版本与当前结果；不得用运行诊断替代正式证据。
