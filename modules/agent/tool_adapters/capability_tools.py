    ...
    if has_business_chain:
        options.append(
            _next_option(
                len(options) + 1,
                "业态缺口与选址预筛",
                "ready_dimensions=poi,h3,population,nightlight,road",
                "选择一个目标业态并生成供给缺口、机会格和候选点位 raw metrics",
    ...
    if has_market_chain:
        options.append(
            _next_option(
                len(options) + 1,
                "细分业态与竞品结构",
                "ready_dimensions=poi,h3",
                "选择目标业态并生成细分 POI、H3 密度和空间缺口 raw metrics",
    ...
    if readiness["road"] and (readiness["h3"] or readiness["poi"]):
        options.append(
            _next_option(
                len(options) + 1,
                "可达性真实性校验",
                f"ready_dimensions=road,{ 'h3' if readiness['h3'] else 'poi' }",
                "对照热点、候选格和步行/驾车可达性 raw metrics",
    ...
    if readiness["nightlight"] and readiness["poi"]:
        options.append(
            _next_option(
                len(options) + 1,
                "夜间活力与业态匹配",
                "ready_dimensions=nightlight,poi",
                "对比夜光热点与 POI 分布 raw metrics",
    ...
    if readiness["population"] and readiness["poi"]:
        options.append(
            _next_option(
                len(options) + 1,
                "客群与服务供给匹配",
                "ready_dimensions=population,poi",
                "对照人口年龄结构和 POI 供给 raw metrics",
