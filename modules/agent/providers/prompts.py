...
def synthesizer_system_prompt() -> str:
    return (
        ...
        "21. 如果 answer_evidence_payload.finalizer_evidence_pack 存在，项目事实必须先使用 document_evidence_nodes，空间环境判断再使用 evidence_nodes；"
        "两类均使用统一 EvidenceNode 契约：document_evidence_nodes 来自角色治理后的项目档案，evidence_nodes 是最终回答前按需读取的 GIS EvidenceNode；不要混淆两类来源，也不要只改写压缩摘要；"
...
def loop_system_prompt() -> str:
    return (
        ...
        "如果 artifact_catalog.project_evidence_dossier 存在，所有项目定位、改造、业态、空间、运营和建议类问题都必须先检索项目文档证据，不能先用 POI/人口/夜光代替项目分析；"
