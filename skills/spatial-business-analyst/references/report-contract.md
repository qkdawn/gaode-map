...
`project-report.md` 是正式模式的唯一读者交付物，不另建情景报告或研究底稿出口。报告内每项重要判断必须标为 `当前建议`、`条件性建议` 或 `暂不建议`，并在证据审计中使用 `直接证据`、`公开检索证据`、`代理推断`、`情景假设`、`待验证` 五类标签。

无法由项目材料直接确认的市场、许可、支付、成本、容量或运营判断，先完成公开网页检索；仍无直接记录时，纳入同一份正式报告的条件性建议。条件性建议必须说明事实输入、参数范围、推演规则或公式、结果范围、反证、停止条件和校准记录；它们不得被表述为真实客流、收入、ROI、许可或已获批实施承诺。

当条件性建议涉及完整市场和产品路径时，其所在章节必须覆盖项目条件、候选客群、市场参数、行为链、产品、业态、容量或经济关系、分期以及校准表。未知数字保留为变量或范围；空间代理指标只能改变供给比较、到达测试、服务重复、入口路径或小规模测试判断，不能直接推出到场、支付或承载。
