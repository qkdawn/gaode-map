...
公开资料的取证、引用和证据边界遵循 `references/research-workflow.md` 的“研究原则与证据等级”。需要补充市场母体、竞品、政策、机构采购或公开价格资料时，使用已注册的 `search_public_web`，并保留标题、链接、日期、适用范围和不可推断边界。正式综合报告的市场发现必须覆盖统计、政策规划、文保档案、片区供给、直接及区域竞品、文化机构或机构采购、公开价格与活动；页面失效或一次查询无结果时继续改写查询和替换来源，不能直接退为后续观察计划。
...
在定位、空间功能和运营方案初稿形成后启动产品市场再校核分析师。定位产品草案必须先提供完整稳定的 `product_inventory`（`status=draft`，逐项包含 `product_id` 与 `name`）；缩减、延后或退出只改变裁决，不删除产品身份。再校核据此校核客群覆盖、容量、价格或价值交换、收入与成本、运营主体和实施分期，保存：
