...
def test_storage_publishes_atomically_and_rejects_tampering(tmp_path):
    ...
    storage.create(
        history_id="history",
        manifest=_manifest(),
        artifact_payloads={"output": "# report"},
        execution_request={"history_id": "history", "analysis_snapshot": {}},
    )
    run_dir = tmp_path / "test-capability" / "run-1"
    assert (run_dir / ".complete").is_file()
    assert (
        storage.read("test-capability", "run-1")["artifacts"][0]["payload"]
        == "# report"
    )
...
def test_storage_copies_upstream_and_rejects_unsafe_filenames(tmp_path):
    ...
    storage.create(
        ...
        artifact_payloads={"output": "# source"},
    ...
    assert not (tmp_path / "test-capability" / "unsafe").exists()
