...
def test_auditor_requires_road_evidence_for_road_spatial_distribution():
    snapshot = _snapshot_with_scope()
    memory = WorkingMemory(
        artifacts={
            "scope_polygon": snapshot.scope["polygon"],
    ...
    assert result.passed is False
    assert "路网概览" in result.required_evidence
    assert "路网概览" in result.missing_evidence

    memory.artifacts["current_road_pattern_analysis"] = {"node_count": 8, "summary_text": "路网画像完整"}
    result = audit_execution(
        question="分析路网集成度和连接度的空间分布，识别低值区域",
        snapshot=snapshot,
        context=build_context_bundle(snapshot),
    ...
    assert "路网概览" not in result.missing_evidence
